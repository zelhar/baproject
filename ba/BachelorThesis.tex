...
\textbf{Spectral Partitioning} \cite{WikipediaGraphPartition, WikipediaSpectralClustering},
...
Consider the graph $G$ and its adjacency matrix $A$. We create the transition
matrix by normalizing it: $T = AD^-1$, and finally we choose a restart parameter
$\alpha$ and create the diffusion
kernel $K$. Theorem \ref{thm:AKTcharacteristics} assures us that $K$ and $T$
have the same eigenvectors and the orders of their corresponding eigenvalues are
the same.
The eigenvalues are all real and therefore the eigenvectors are real as well.
And the eigenvalues of $K$ are all non-negative.

The eigenvector which corresponds to the biggest eigenvalue $1$ of $T$ is all
positive. Any other eigenvector must contain both positive and negative
components, otherwise its eigenvalue must be $1$ and there is only one and
unique state which is an eigenvector. Therefore we consider the eigenvector
which corresponds to the second largest eigenvalue (They are also known as the
\textbf{Fiedler} eigen-pair).

We say that $x \in \{1,-1\}^n$ is a \textbf{non-trivial 2-partition} of $V = \{1
\dots n\}$ if $|\sum x_i| \lt n$. If we want to find the partition that
maximizes $Kx$ 
...
\caption{A T cluster partition}
%\label{fig:examplecoolwarm2cluster}
\end{subfigure}
%add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. 
%(or a blank line to force the subfigure onto a new line)
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=\textwidth]{example_spectralKK2clustering.png}
\caption{A K cluster partition}
%\label{fig:examplecoolwarm3cluster}
\end{subfigure}
%add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. 
%(or a blank line to force the subfigure onto a new line)
\caption{The result of running the algorithm set for 2-partition and
respectively a 3-partition. The interesting node is 12. In both runs 12 chooses
to associate with the biggest clique because node 4 of that clique is the one
which receives the most heat from 12}
%\label{fig:exampleCoolWarmClustering}
...
%(or a blank line to force the subfigure onto a new line)
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=\textwidth]{Karate_spectralKK2clustering.png}
\caption{A K cluster partition}
%\label{fig:examplecoolwarm3cluster}
\end{subfigure}
%add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. 
