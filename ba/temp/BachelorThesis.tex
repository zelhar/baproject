...
This Thesis deals with applircations of propagation methods for
protein function prediction. Our goal is to propose a prediction
algorithm.  Its input is a
bidirectional graph and a labeling of a subset of the
vertices \textemdash that's the partial classification which we wish
to extend. The algorithm outputs for each unlabeled vertex  a
suggested suitable label, and the label must be one of the existing
labels.
We are going to require just one label for each vertex.
That is for the sake of simplification, although
in reality when we deal with protein-protein interaction networks
(PPI), each vertex (protein) can have multiple labels (functions or
annotations in the Jargon).
We call this problem the partially labeled
classification problem (PLC) as it was named by
\textcite{szummer2002partially}.

Our input network is a PPI network, which is a bidirectional graph
where each vertex represents a protein type in the cells of a certain
organism and each edge represent some sort of an observed
'interaction' between two proteins that is unlikely to be taking place
...
yet they are observed in the test tube due to the method that
produces the PPI data.

Suppose that some of the proteins are well researched and the life science guys
have provided us with further information about their biological
functions. This
...
unlabeled. We want to extend this labeling to all the nodes, by
guessing for each unlabeled vertex which label could fit it
best.

Estimating the accuracy of the prediction is another issue of
concern. In
...
This gives us an estimation of the total accuracy.
We also measured the sensitivity, specificity, and precision of our
proposed method for each functional group.

We would also like to estimate or score the likelihood for each
specific prediction (perhaps as part of a future paper or in the master
thesis). We would need for example to come up with a statistical way to quantify
the posterior probability that a vertex has a certain label
\cite{szummer2002partially}. 

We use propagation methods in attempt to find a prediction algorithm
for the PLC. Specifically we use random walk with restart (RWR)
approach. Essentially what we do is picking an unlabeled vertex and
starting randomly walking from it with restarts at a certain
probability back to the starting vertex. In this walk we sometimes
visit labeled vertices. We predict that the label that we most
frequently encounter in this walk is the most likely label of the
starting vertex.
...
they reached specificity and sensitivities of 45-64\% but this is not
apples to apples comparison with our proposed method.

The other paper is \textcite{szummer2002partially} and it is purely
mathematical and from it we adapted the name \textbf{'partially labeled
...
walk with restart  in graphs. We think that this theoretical introduction is
...
a great deal of overlap between the PLC problem and community structure. In
...
If the labels don't have characteristics of clusterings or community formation
then these RWR methods is basically useless.
...
\begin{framed}
\centering
\includegraphics[width=\textwidth]{alpha_fraq_graph.png}
