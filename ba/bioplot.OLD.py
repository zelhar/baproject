...
def decision_function(v, G, group_membership_list, know_unknown_list):
    ...
    Input know_unknown_list: boolean 1d array which
    ...
    for g in range(len(group_names)):
        x = q[np.array(group_membership_list) == group_names[g]] 
        testscores[g] = x.sum()
...
simple_decision_by_neighbors(3, G, group, know_unknown)
...
def decision_by_known_groups(G, group_membership_list, know_unknown_list):
    """
    """
    all_nodes = np.arange(len(G.nodes))
    known_nodes = all_nodes[know_unknown_list]
    unkown_nodes = all_nodes[know_unknown_list == False]
    group_names = np.unique(group_membership_list)
    biases = np.zeros((len(groups),len(know_unknown_list)))
    for g in range(len(group_names)):
        x = np.array(group_membership_list) == group_names[g]
        bias[g] = x * know_unknown_list 
