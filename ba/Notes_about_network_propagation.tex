...
row, and $B$ has a positive entry in evert column. Then $(A+B) \gt 0$ and
therefore $A+B$ is regular. The proof is trivial. What's important to our case
is that the adjacency matrix of a connected graph has the property of $B$ and
the matrix like $S$ has the property of $A$. Therefore the composite matrix is
regular fro any arbitrary restart state $s$.
